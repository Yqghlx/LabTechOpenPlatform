...
const { redisClient, connectRedis, CHANNELS } = require('./redisClient');
...
app.post('/api/systems/:systemId/control', async (req, res) => {
    ...
    try {
        ...
        await redisClient.publish(`${CHANNELS.CONTROL_COMMANDS}:${systemId}`, message);
...
async function setupStateListener() {
    ...
    await subscriber.subscribe(CHANNELS.STATE_UPDATES, (message) => {
        try {
            ...
            redisClient.set(redisKey, JSON.stringify(state), { EX: 3600 });
...
async function startServer() {
    ...
    app.listen(PORT, () => {
        console.log(`Redis-Only Central Hub listening on port ${PORT}`);
    });
}
